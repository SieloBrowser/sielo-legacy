...
namespace ndb
{
    ...
    struct custom_type { using ndb_type = fundamental_type; };

    template<class T, class Database = global_database, class Group = typename Database::group, class Engine = typename Database::engine>
    struct is_custom_type
    {
        static constexpr bool value = !std::is_same_v<typename custom_type<T, Database, Group, Engine>::ndb_type, ndb::fundamental_type>;
    };
